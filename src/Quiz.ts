...
type Problems = AdditionProblem|SubtractionProblem|MultiplicationProblem|DivisionProblem|FactorialProblem|PercentageProblem|ExponentProblem
...
export class Quiz {
  ...
  init(): void {
    ...
    this.responseElement = document.getElementById('response') as HTMLInputElement;

    document.addEventListener("keydown", (e: KeyboardEvent): void => {
      const regex: RegExp = /^[0-9.,\-]+$/;
      if (regex.test(e.key)) {
        this.responseElement.innerHTML = "";
        if (document.activeElement.tagName != "INPUT") this.answerElement.value += e.key;
...
`;
  ...
  selectRandomProblem(): void {
    ...
    let problemResponse: { question: null|string; answer: null|number|string };
  ...
  checkAnswer(): void {
    ...
    if (parseInt(submittedAnswer, 10) === this.problem.answer) {
      // Correct
  ...
  trackResponse(response: 'c'|'i' = 'c'): void { // add response type to responseTracker
    if (response == 'c') {
